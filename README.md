...
_(Lưu ý: Container ở trên chỉ bao phủ phần Database & Backend. Bạn vẫn cần qua thư mục `frontend/` chạy `npm run dev` như bước 3 để truy cập giao diện gốc)._

## Hướng dẫn sử dụng

1. **Đăng ký**: Truy cập trang web, điền Họ tên, Email và Mật khẩu, nhấn **Nhận mã xác thực**. Nhập mã OTP 6 số được gửi về email để hoàn tất đăng ký.
2. **Đăng nhập**: Truy cập trang Đăng nhập, nhập email và mật khẩu. Nếu quên mật khẩu, chọn **Quên mật khẩu** để nhận OTP khôi phục qua email.
3. **Quản lý tài khoản**: Từ Dashboard, nhấn vào avatar góc trên phải hoặc chọn menu **Thông tin tài khoản** để xem và cập nhật hồ sơ cá nhân.
4. **Quản lý Quiz**: Từ Dashboard, chọn menu **Quản lý Quiz** để xem danh sách Quiz, thêm mới, chỉnh sửa hoặc xóa.
5. **Tạo và biên soạn Quiz**: Nhấn **Tạo Quiz**, điền thông tin cài đặt (tiêu đề, thời gian, thang điểm) rồi **Lưu nháp**. Sau đó chuyển sang tab **Câu hỏi** để thêm, sửa, xóa câu hỏi — hỗ trợ import từ Excel hoặc chọn từ Ngân hàng câu hỏi.
6. **Lập lịch thi**: Từ QuizEditor, chọn tab **Lập lịch** để đặt thời gian mở/đóng bài, giới hạn số thí sinh và số lượt thi tối đa mỗi người.
7. **Xuất bản và chia sẻ**: Nhấn **Xuất bản Quiz** để công khai bài thi. Vào mục **Trạng thái & Mã PIN**, nhấn **Tạo mã mới** để sinh mã PIN rồi sao chép chia sẻ cho thí sinh.
8. **Tham gia Quiz**: Từ Dashboard, nhập mã quiz vào ô **Nhập mã quiz** rồi nhấn **Tham gia**, hoặc chọn trực tiếp một quiz đang mở trong danh sách.
9. **Làm bài thi**: Trả lời các câu hỏi trong thời gian quy định. Chuyển tab quá số lần giới hạn sẽ bị nộp bài tự động. Điểm được chấm và hiển thị ngay sau khi nộp.
10. **Lịch sử làm bài**: Từ Dashboard, chọn menu **Lịch sử tham gia** để xem tất cả các lần làm bài, điểm số và xem lại chi tiết từng câu trả lời.
11. **Kết quả và báo cáo**: Từ Dashboard, chọn menu **Kết quả thi** để xem kết quả chi tiết từng thí sinh, mở **Bảng xếp hạng** và **Thống kê câu hỏi**. Hỗ trợ xuất báo cáo dạng Excel/PDF và xuất bộ đề (Đề thi / Đáp án / Lời giải / Ôn tập).
