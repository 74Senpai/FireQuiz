...
export const getSomethingFromDb = async (quizId) => {
  const [rows] = await pool.execute("SELECT * FROM somethings WHERE id = ?", [
    quizId,
  ]);
  return rows[0];
};
...
describe("SomeService - getSomething", () => {
  ...
  it("Nên throw Error (404) nếu repository không tìm thấy dữ liệu", async () => {
    ...
    // Validate throws
    await expect(someService.getSomething(1, { id: 2 })).rejects.toThrow(
  ...
  it("Nên trả về object kết quả nếu DB và quyền xác thực đầy đủ", async () => {
    ...
    jest
      .spyOn(someRepository, "getSomethingFromDb")
...
## 5. Quy định về Logging

Để đảm bảo hệ thống có thể theo dõi và gỡ lỗi hiệu quả, dự án sử dụng thư viện `winston` thông qua tiện ích `logger` tại `src/utils/logger.js`.

### Các Nguyên Tắc Logging:

1. **Tuyệt đối không dùng `console.log()` hoặc `console.error()` trực tiếp**: Hãy sử dụng `logger` thay thế để log có format chuẩn (timestamp, level) và có thể lưu file.
2. **Tự động log Request**: Mọi yêu cầu API đều đã được tự động ghi lại bởi middleware `requestLogger` (Method, URL, Status, Response Time). Bạn không cần log thủ công từng request đơn giản.
3. **Sử dụng đúng Level**:
   - `logger.info("Message")`: Log các hoạt động bình thường, quan trọng của hệ thống (Ví dụ: Server start, DB connected, User created).
   - `logger.error(err)` hoặc `logger.error("Message", err)`: Log các lỗi xảy ra. Tích hợp trong `errorHandler` để bắt lỗi tập trung.
   - `logger.warn("Message")`: Log các tình huống nghi vấn, không quá nghiêm trọng nhưng cần lưu ý.
   - `logger.debug("Message")`: Log chi tiết phục vụ quá trình phát triển (Ví dụ: In giá trị biến trung gian). Các log này thường chỉ hiện ở môi trường development.
...
export const someAction = async (data) => {
  logger.info(`Đang thực hiện action với data: ${data.id}`);
  // ... logic ...
  if (success) {
    logger.debug(`Action thành công cho user: ${data.userId}`);
  }
};
```
