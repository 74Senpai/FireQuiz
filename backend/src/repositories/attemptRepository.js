...
export const getQuestionsAndAnswersByQuizId = async (conn, quizId) => {
  const [rows] = await conn.execute(
    `SELECT q.id as question_id, q.content as question_content, q.type as question_type, q.explanation as question_explanation,
...
export const bulkInsertAttemptQuestions = async (conn, questionsData) => {
  ...
  const [result] = await conn.query(
    `INSERT INTO attempt_questions (quiz_attempt_id, content, type, explanation) VALUES ?`,
    [questionsData]
  );
  return result;
};
...
export const getAttemptSnapshot = async (attemptId) => {
  const [questions] = await pool.execute(
    `SELECT id, content, type, explanation FROM attempt_questions WHERE quiz_attempt_id = ?`,
    [attemptId]
  );
