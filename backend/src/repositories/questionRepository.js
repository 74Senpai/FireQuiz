...
export const create = async (data, tx = pool) => {
  const { content, type, quizId, mediaUrl, bankQuestionId } = data;

  logger.info(`questionRepository.js - Creating question - content: ${content}, type: ${type}, quizId: ${quizId}`);
  const sql = "INSERT INTO questions(content, type, media_url, quiz_id, bank_question_id) VALUES (?, ?, ?, ?, ?);";
  const [row] = await tx.execute(sql, [content, type, mediaUrl ?? null, quizId, bankQuestionId ?? null]);
  return row.insertId;
};
